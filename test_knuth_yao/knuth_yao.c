...
bool pre_knuthyao(float mu, float sigma, node* root, int x_values[], uint32_t *binary_p_mat, node ** expansion_margin){
    ...
    for(int i=0;i<Pmat_row_size;i++)
    {
        ...
        // int x=(i-(Pmat_row_size-1)/2)+(int)mu;
        x_values[i]=x;
        P_mat[i]= f_exp(-(x-mu)*(x-mu)*isigma*isigma/2); //必须替换成自己写的exp
...
int knuth_yao_sampling(uint64_t random_bits, node * tree)
{
    ...
    {   
        printf("i = %d, random_bits&index)&index = %lu, current_tree->type = %d\n ", i, (random_bits&index)&index&1, current_tree->type); 
        //判断走到头没有
        if(current_tree->type!=255){
            //走到头了
            //sanity check
            int length=Pmat_row_size;
            // return (current_tree->type)%length;
            return (current_tree->type); //%length;
        }

        //接着走，取出bit并判断0还是1
        if((random_bits&index)&index) //revise
        {
            //向右走
