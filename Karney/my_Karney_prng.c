...
// double generate_uniform_random(prng *p, double x) { //���������[0, x]
...
int DiscreteGaussian_Karney(void *ctx, float mean, float stddev) {
	int k, s;
	...
	while(1) {
		...
		int j = generate_uniform_random(&spc->p, ceil(stddev)); // С������ 
		float x = x0 + j / stddev;
